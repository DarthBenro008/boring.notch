...
class WebcamManager: NSObject, ObservableObject {
    ...
    private let sessionQueue = DispatchQueue(label: "CustomNotch.WebcamManager.SessionQueue", qos: .userInitiated)
        ...
        sessionQueue.async { [weak self] in
            ...
            do {
                ...
                guard session.canAddInput(videoInput) else {
                    throw NSError(domain: "CustomNotch.WebcamManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "Cannot add video input"])
                }
